...
export const buildDeductibleLedgerXlsx = (ledger: TreasuryDeductibleLedger) => {
  ...
  const rows = ledger.rows.map((row) => ({
    ...
    "Supplier NCF": row.supplierNcf ?? "",
    "DGII type": row.dgiiExpenseType ?? "",
    Concept: row.concept ?? "",
    ...
    "Withholding type": row.withholdingType ?? "",
    "Withholding rate": row.withholdingRate ?? "",
    "Withholding amount": row.withholdingAmount ?? "",
    "Fiscal period": row.fiscalPeriod ?? "",
...
export const createDeductibleLedgerPdf = async (ledger: TreasuryDeductibleLedger) => {
  ...
  const columns = [
    ...
    { label: "Counterparty / RNC", width: 120 },
    { label: "NCF", width: 76 },
    { label: "Concept", width: 128 },
    { label: "DGII / Category", width: 92 },
  ...
  for (const row of ledger.rows) {
    ...
    const values = [
      ...
      row.dgiiExpenseType || row.expenseCategory || "",
