...
const tableConfigs: Array<{ table: TreasuryPullTable; cursorColumn: string; idColumn: string }> = [
  { table: "bank_accounts", cursorColumn: "updated_at", idColumn: "id" },
  { table: "bank_statement_imports", cursorColumn: "created_at", idColumn: "id" },
  { table: "bank_transactions", cursorColumn: "created_at", idColumn: "id" },
  { table: "transaction_annotations", cursorColumn: "updated_at", idColumn: "transaction_id" },
  { table: "transaction_project_allocations", cursorColumn: "updated_at", idColumn: "id" },
  { table: "transaction_links", cursorColumn: "updated_at", idColumn: "id" },
  { table: "counterparty_rules", cursorColumn: "updated_at", idColumn: "id" },
];
...
export const useTreasuryPull = () => {
  ...
  useEffect(() => {
    ...
    const runOnce = async () => {
      ...
      try {
        for (const { table, cursorColumn, idColumn } of tableConfigs) {
          ...
          for (let page = 0; page < MAX_PAGES_PER_TABLE; page += 1) {
            ...
            query = applyCompositePullCursor(query, cursor, cursorColumn, idColumn);
