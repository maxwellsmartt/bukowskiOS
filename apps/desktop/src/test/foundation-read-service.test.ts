...
describe("foundation read service", () => {
  it("hydrates shell, imported assets and finance snapshots from the local foundation database", () => {
    ...
    const auroraDetail = reads.getProjectDetail("project-aurora");
    expect(auroraDetail.project?.name).toBe("Aurora Campaign");
    expect(auroraDetail.project?.client).toBe("Altura");
    expect(auroraDetail.assets.length).toBeGreaterThan(0);
    expect(auroraDetail.incidents.length).toBeGreaterThan(0);
    expect(auroraDetail.incidents[0]?.assetCode).toBeTruthy();
    expect(auroraDetail.incidents[0]?.asset).toBeTruthy();
    expect(auroraDetail.incidents[0]?.asset).not.toBe(auroraDetail.incidents[0]?.assetCode);
    expect(auroraDetail.metrics).toHaveLength(4);
